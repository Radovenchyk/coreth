...
func (v blockValidator) SyntacticVerify(b *Block, rules params.Rules) error {
	...
	headerExtra := types.HeaderExtras(ethHeader)
	if rulesExtra.IsApricotPhase1 {
		if hash := types.CalcExtDataHash(types.BlockExtData(b.ethBlock)); headerExtra.ExtDataHash != hash {
	...
	if types.BlockVersion(b.ethBlock) != 0 {
		return fmt.Errorf("invalid version: %d", types.BlockVersion(b.ethBlock))
	}
	...
	if rulesExtra.IsApricotPhase4 {
		...
		if rulesExtra.IsApricotPhase5 {
			if headerExtra.ExtDataGasUsed.Cmp(params.AtomicGasLimit) == 1 {
